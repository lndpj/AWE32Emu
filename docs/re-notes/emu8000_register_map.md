...
Krok 6 zapisuje `2400h` (= CVCF) a `3400h` (= VTFT) hodnotou `FFFFFFFF`,
ostatni nulou - tj. hlasy startuji s plnou hlasitosti/otevrenym filtrem
v aktualnich i cilovych registrech, ale s vypnutym envelope enginem
(DCYSUSV=0080h).
...
- Presne casove konstanty envelope (tabulky attack/decay/release) - hledat
  v `SBAWE32.DRV` (Windows AWE32 MIDI driver, NE format, 45 kB)
- Prevod SoundFont generatoru -> registry (tamtez)
- Krivka filtru (IFATN hi8 -> Hz) a Q (CCCA bity 31..28)
- Vyznam CCCA control bitu 27..24
